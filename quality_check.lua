...
-- busca monosilabos que NUNCA llevan tilde
	...
	if text:match("¿") then
		if text:match("%aue")
		or text:match("%auien")
		or text:match("Quienes")
		or text:match("cual%s")
		or text:match("cuales%s")
		or text:match("cuant%a%s")
		or text:match("cuant%as%s")
		or text:match("cuanto")
		or text:match("cuy%a")
		or text:match("cuy%as")
	...
	if res["apertura"] then
		if text:match("¿") then
			if not text:match("%?") then effect=effect.." [¿falta cerrar una interrogación?]" interr=interr+1 end
		end
		if text:match("%?") then
			if not text:match("¿") then effect=effect.." [¿falta abrir una interrogación?]" interr=interr+1 end
		end
		if text:match("¡") then
			if not text:match("!") then effect=effect.." [¿falta cerrar una exclamación?]" interr=interr+1 end
		end
		if text:match("!") then
			if not text:match("¡") then effect=effect.." [¿falta abrir una exclamación?]" interr=interr+1 end
		end
	end	
	...
	visible2w=visible.."."
		...
		if a==b and not a:match("^%u") then effect=effect.." [doble letra]" dword=dword+1 end
		end
    ...
    if colorspace=="TV.601" then report=report.."EL ESPACIO DE COLOR ES TV.601. ¡Usa TV.709 por el amor de dios!\n" end
    ...
    if dis~=0 then report=report.."Líneas con etiquetasinconexas... "..dis.."\n" end
...
function konfig(subs, sel)
	...
	{x=1,y=16,width=1,height=1,class="checkbox",name="halfsec",label="Comprueba si hay líneas que duren menos de 0.5s",value=true,hint="bur over 1 frame and over 8 characters"},
	...
	{x=1,y=21,width=1,height=1,class="label",label="Utilidades medio útiles que spamean la coulmna de efecto:",value=true},
	{x=1,y=22,width=1,height=1,class="checkbox",name="monosilabos",label="Comprueba si hay posibles monosilabos sin tilde diacrítica",value=false},
	{x=1,y=23,width=1,height=1,class="checkbox",name="monosilabost",label="Comprueba si hay posibles monosilabos con tilde diacrítica",value=false},
	{x=1,y=24,width=1,height=1,class="checkbox",name="interrogativas",label="Comprueba si hay posibles pronombres interrogativos sin tilde",value=false},
	...
	{x=2,y=15,width=2,height=1,class="checkbox",name="uselesstyle",label="Lista los estilos que no se usan",value=false},
	{x=2,y=16,width=2,height=1,class="checkbox",name="sauro",label="Count lines with faggosaurosis",value=true},
	
	{x=1,y=17,width=2,height=1,class="label",label=""},
	{x=1,y=25,width=3,height=1,class="label",label="Este script es para ayudarte a ver fallos. Si lo estás usando como sustituto de un QC eres un idiota."},
	
	}
	buttons={">QC","Limpiar QC","Dial 5","Marcar todos","Que le den a esta mierda"}
